...
from pose_utils import normalize, look_at, poses_avg, recenter_poses, to_float, generate_spiral_cam_to_world, \
    generate_spherical_cam_to_world, flatten
...
class NeRFDataset(Dataset):
    ...
    def generate_rays(self):
        ...
        directions = ((camera_directions[None, ..., None, :] * self.cam_to_world[:, None, None, :3, :3]).sum(
...
class LLFF(NeRFDataset):
    ...
    def generate_rays(self):
        ...
        super().generate_rays()
        ndc_origins, ndc_directions = convert_to_ndc(self.rays.origins, self.rays.directions, self.focal, self.w,
                                                     self.h)
        mat = ndc_origins
        # Distance from each unit-norm direction vector to its x-axis neighbor.
...
class nerf360(NeRFDataset):
    ...
    def generate_training_poses(self):
        ...
        self.h, self.w = images.shape[1:3]
        self.n_poses = images.shape[0]
        self.near, self.far = bds.min() * .9, bds.max() * 1.1
